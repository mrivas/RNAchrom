...
def getGenomicIntervals(line,distance):
	line=line.strip("\n")
	fields=line.split("\t")
	iv_dna= HTSeq.GenomicInterval(fields[9],int(fields[10]),int(fields[11]),".")
	iv_rna= HTSeq.GenomicInterval(fields[0],int(fields[1]),int(fields[2]),".")
	longRange = False

	# if RNA-end is not sitting on top of an exon, utr3, or utr5 region: skips
	if not ( fields[7] == "exon" or fields[7] == "utr3" or fields[7] == "utr5" ):
