...
parser = argparse.ArgumentParser(description='Counts the number of long-range interactions per gene. Only RNA ends counts per gene')
...
def getLongRangeInteractions(aFile,distance,geneIV_ID):
	...
	for line in open(aFile,"r"):
		iv_rna, longRange = getGenomicIntervals(line,distance)
...
counts = getLongRangeInteractions(aFile,distance,geneIV_ID) 
...
for geneID in geneID_IV:
	...
	coord = geneID_IV[geneID].chrom+":"+str(geneID_IV[geneID].end)+"-"+str(geneID_IV[geneID].start)+";"+geneID_IV[geneID].strand
	if geneID in counts:
		print >>out,  geneID+"\t"+str(counts[geneID])+"\t"+str(length)+"\t"+bioType[geneID]+"\t"+coord
	else:
		print >>out, geneID+"\t0\t"+str(length)+"\t"+bioType[geneID]+"\t"+coord
out.close()
